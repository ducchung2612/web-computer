...
function addKhungSuaSanPham(masp) {
    ...
    var company = ["Lenovo", "Asus", "Acer", "Gigabyte", "Mac", "Dell","HP", "MSI"];
...
function addTableDonHang() {
    ...
    for (let u of listUser) {
        for (let dh of u.donhang) {
            let info = dh.thongTinGiaoHang || {};
            ...
            if (info.paymentMethod) {
                if (info.paymentMethod === 'cod') {
                    paymentText = 'Tiền mặt khi nhận hàng';
                    paymentClass = 'payment-cod';
                    paymentIcon = '<i class=\"fa fa-money\"></i> ';
                } else if (info.paymentMethod === 'bank') {
                    paymentText = 'Chuyển khoản ngân hàng';
                    paymentClass = 'payment-bank';
                    paymentIcon = '<i class=\"fa fa-university\"></i> ';
                } else if (info.paymentMethod === 'e-wallet') {
                    paymentText = 'Ví điện tử';
                    paymentClass = 'payment-ewallet';
                    paymentIcon = '<i class=\"fa fa-mobile\"></i> ';
                } else {
                    paymentText = info.paymentMethod;
                    paymentClass = '';
                    paymentIcon = '';
                }
            }
            ...
            let spText = '';
            if (dh.sp && dh.sp.length > 0) {
                spText = dh.sp.map(sp => {
                    let p = timKiemTheoMa(list_products, sp.ma);
                    return p ? `${p.name} [${sp.soluong}]` : '';
                }).join('<br>');
            }
            // Tổng tiền
            let totalPrice = 0;
            if (dh.sp && dh.sp.length > 0) {
                for (let sp of dh.sp) {
                    let p = timKiemTheoMa(list_products, sp.ma);
                    let price = (p && p.promo && p.promo.name == 'giareonline' ? p.promo.value : (p ? p.price : 0));
                    totalPrice += stringToNum(price) * sp.soluong;
                }
            }
            s += `<tr>
                <td style='width:4%;text-align:center;'>` + (stt++) + `</td>
                <td style='width:10%;text-align:center;'>` + dh.ngaymua + `</td>
                <td style='width:7%;text-align:center;'>` + u.username + `</td>
                <td style='width:15%;'>` + address + `</td>
                <td style='width:8%;text-align:center;'>` + phone + `</td>
                <td style='width:8%;'>` + note + `</td>
                <td style='width:10%;text-align:center;' class=\"${paymentClass}\">` + paymentIcon + paymentText + `</td>
                <td style='width:15%;'>` + spText + `</td>
                <td style='width:8%;text-align:right;'>` + numToString(totalPrice) + `</td>
                <td style='width:7%;text-align:center;'>` + new Date(dh.ngaymua).toLocaleString() + `</td>
                <td style='width:3%;text-align:center;'>
                    <select class='trangthai-select' data-madon='`+dh.ngaymua+`' style=\"padding:2px 4px;min-width:70px;max-width:100px;font-size:0.95em;border-radius:6px;\">
                        ${trangThaiList.map(stt => `<option value=\"${stt}\"${dh.tinhTrang===stt?' selected':''}>${stt}</option>`).join('')}
                    </select>
                </td>
            </tr>`;
            TONGTIEN += totalPrice;
...
// Thêm hàm cập nhật trạng thái hàng loạt
window.capNhatTatCaTrangThaiDonHang = function() {
    var selects = document.querySelectorAll('.donhang .trangthai-select');
    var listUser = getListUser();
    let changed = false;
    selects.forEach(sel => {
        var madon = sel.getAttribute('data-madon');
        var newStatus = sel.value;
        for (let u of listUser) {
            for (let dh of u.donhang) {
                if (dh.ngaymua.toString() === madon && dh.tinhTrang !== newStatus) {
    ...
    if (changed) {
        ...
        addTableDonHang();
        alert('Đã cập nhật trạng thái các đơn hàng!');
    } else {
